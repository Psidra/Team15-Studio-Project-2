...
{
	float rate_of_change = (1/x_distance * 9) * (-g * pow(cos((pi / 180) * init_angle), 2)); // jesus this was annoying to type
...
{
	float rate_of_change = (1 / x_distance * 3) * (-g * pow(cos((pi / 180) * init_angle), 2));
